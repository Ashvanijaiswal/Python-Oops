...
class RunningApp():
    ...
    def ingest_raw_data(self,spark):
        raw_data= spark.read.format("json").schema(self.getSchema()).load(f"{self.base_data_dir}/invoices.json")
        return raw_data

    def explodeInvoiceData(self, raw_data):
        return raw_data.selectExpr("InvoiceNumber", "CreatedTime", "StoreID", "PosID",
                                      "CustomerType", "CustomerCardNo","PaymentMethod", "DeliveryType", "DeliveryAddress.City",
    ...
    def appendIntoDataLake(self,spark, flattenDF):
       flattenDF.write.format("csv").mode("append") \
         .option("header", "true")\
         .save(f"{self.base_data_dir}/output")
...
if __name__=="__main__":
    try:
        ...
        ob.appendIntoDataLake(spark, flattenDF)
